    ...
    fetch("/api/vehicles")
      ...
      .then((d) => {
        if (d.success && d.vehicles && d.vehicles.length > 0) {
          const mapped: Vehicle[] = d.vehicles.map((v: any) => ({
            slug: v.slug,
            name: v.name,
            examples: Array.isArray(v.examples) ? v.examples : [v.name],
            category: v.category as VehicleCategory,
            seats: Number(v.seats || 4),
            bags: Number(v.bags || 2),
            luggage: v.luggage || "2 bags",
            bestFor: v.description || "Comfortable trip",
            features: Array.isArray(v.features) ? v.features : ["AC", "Chauffeur driven"],
            summary: v.description || "Comfortable vehicle",
            image: v.image || "/images/fleet/car-sedan.jpg",
            ac: true,
            tripTypes: ["Local", "Airport", "One-Way", "Round Trip", "Outstation", "Corporate", "Group Travel"],
            startingFrom: Number(v.price_per_km || 14),
          }));
...
function VehicleDetailsModal({ v, onClose }: { v: Vehicle; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 md:p-8 shadow-2xl">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 rounded-full border border-border p-2 text-slate-500 hover:bg-slate-100"
          aria-label="Close modal"
        >
          <X className="h-5 w-5" />
        </button>

        <div className="relative aspect-[16/10] w-full overflow-hidden rounded-2xl bg-slate-100 mb-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-heading text-2xl md:text-3xl text-[#12213b]">{v.name}</h2>
            <p className="mt-1 text-xs text-muted-foreground">{v.examples.join(" · ")}</p>
          </div>
          {v.startingFrom && (
            <div className="text-right shrink-0">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Starting from</p>
              <p className="font-heading text-2xl text-[color:var(--color-navy)]">₹{v.startingFrom}<span className="text-xs font-normal text-muted-foreground">/km</span></p>
            </div>
          )}
        </div>

        <p className="mt-4 text-sm text-foreground/80 leading-relaxed">{v.summary}</p>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3 rounded-2xl bg-slate-50 p-4 text-center">
          <div>
            ...
            <span className="text-sm font-bold text-[#12213b]">{v.seats} Passengers</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Luggage</span>
            <span className="text-sm font-bold text-[#12213b]">{v.bags} Bags</span>
          </div>
          <div>
            <span className="text-xs text-muted-foreground block">Climate</span>
            <span className="text-sm font-bold text-[#12213b]">{v.ac ? "Air Conditioned" : "Non-AC"}</span>
          </div>
        </div>
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {v.features.map((f) => (
              <span key={f} className="inline-flex items-center gap-1.5 rounded-full bg-[#0E6B50]/10 px-3 py-1 text-xs font-semibold text-[#0E6B50]">
        ...
        <div className="mt-6">
          <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Best Suited For</h4>
          <p className="text-sm text-slate-700">{v.bestFor}</p>
        </div>

        <div className="mt-8 flex flex-col gap-3 sm:flex-row border-t border-border pt-6">
          <a
            href={waUrl}
            target="_blank"
            rel="noreferrer noopener"
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-[color:var(--color-emerald)] px-6 py-3 text-sm font-bold text-white shadow hover:brightness-110"
          >
          ...
          <Link
            ...
            onClick={onClose}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-full border-2 border-[color:var(--color-navy)] bg-white px-6 py-3 text-sm font-bold text-[color:var(--color-navy)] hover:bg-slate-50"
          >
            View Full Page Details <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </div>
