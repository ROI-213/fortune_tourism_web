    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-black uppercase tracking-wider text-slate-300 flex items-center gap-2">
          <Palmtree className="w-4 h-4 text-purple-400" /> Select Your Tour Package
        </h3>
        <p className="text-xs text-slate-400">Choose a curated South India holiday package or describe your custom trip in the notes below.</p>
            ...
            return (
              <button
                ...
                className={`text-left rounded-2xl border overflow-hidden transition-all ${
                  isSelected
                    ? "border-purple-500/80 ring-2 ring-purple-500/30 shadow-lg shadow-purple-500/10"
              ...
              >
                <div className="relative h-28">
                  ...
                  <div className="absolute bottom-2 left-3 right-3">
                    <div className="text-sm font-bold text-white leading-tight">{pkg.title}</div>
                  </div>
                  {isSelected && (
                ...
                <div className="p-3 flex items-center justify-between">
                  <div>
                    <span className="text-[11px] text-slate-400">{pkg.duration}</span>
      ...
      {formData.package_slug && (
        <>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-2">Journey Type</label>
              ...
              <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
                    <MapPin className="w-4 h-4 text-purple-400" /> Your Tour Route
                  </span>
                  <button
                    type="button"
                    onClick={addDestination}
                    className="text-xs font-bold text-purple-400 flex items-center gap-1 bg-purple-500/10 px-2.5 py-1 rounded-lg border border-purple-500/20"
                  >
                ...
                <div className="space-y-2">
                  {destinations.map((d, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-[10px] font-bold text-purple-400 shrink-0">
                        {idx + 1}
                      </div>
                      <input
                        type="text"
                        ...
                        className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-1.5 text-xs text-slate-100 focus:ring-2 focus:ring-purple-500"
                      />
                ...
                <div className="pt-2 border-t border-slate-800 text-[11px] text-slate-400 flex flex-wrap items-center gap-1.5">
                  ...
                  {destinations.map((d, i) => (
                    <React.Fragment key={i}>
                      <span className="bg-slate-900 px-2 py-0.5 rounded text-purple-300 font-medium">{d}</span>
                      {i < destinations.length - 1 && <ArrowRight className="w-3 h-3 text-slate-600" />}
                    </React.Fragment>
                  ))}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">
                <Calendar className="w-3.5 h-3.5 inline mr-1 text-amber-400" /> Travel Date *
              </label>
              <input
                type="date"
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">Number of Days</label>
              <input
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 block mb-1">Number of Nights</label>
              <input
                type="number" readOnly
                value={Math.max(0, Number(formData.number_of_days || 2) - 1)}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-400 cursor-not-allowed"
              />
            </div>
          </div>
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 space-y-3">
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-300 flex items-center gap-2">
              <Users className="w-4 h-4 text-purple-400" /> Group Members
            </h3>
                ...
                <div key={key} className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <button
                      ...
                      className="w-8 h-8 rounded-xl bg-slate-800 border border-slate-700 text-slate-200 font-bold text-lg flex items-center justify-center hover:bg-slate-700"
                    >
                      −
                    </button>
                    <span className="w-8 text-center font-black text-lg text-slate-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                <Car className="w-3.5 h-3.5 text-amber-400" /> Vehicle Preference
              </label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                <Hotel className="w-3.5 h-3.5 text-purple-400" /> Hotel / Stay Preference
              </label>
              <div className="grid grid-cols-2 gap-2">
                {HOTEL_TIERS.map((h) => (
                  <button
                  ...
                  >
                    <div>{h.id}</div>
                    <div className={`text-[10px] font-normal ${(formData.hotel_tier || "Standard") === h.id ? "text-purple-200" : "text-slate-500"}`}>
                      {h.desc.split(" — ")[0]}
                    </div>
                  </button>
                ))}
