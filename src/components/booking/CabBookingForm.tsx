...
export function CabBookingForm({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* LOCAL HOURLY PACKAGES (Only show 2 options dropdown and proceed) */}
      {tripType === "Local" ? (
        <div className="space-y-4 pt-2">
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5 mb-2">
              <Clock className="w-4 h-4 text-amber-600" /> Select Hourly Package *
            </label>
            <select
              value={localPackage}
              onChange={(e) => {
                const val = e.target.value;
                onChange("local_package", val);
              }}
              className="w-full bg-white border-2 border-amber-500 rounded-2xl px-4 py-3.5 text-sm font-black text-slate-900 focus:ring-2 focus:ring-amber-500 outline-none shadow-sm cursor-pointer"
            >
              <option value="4hr_40km">4 Hours / 40 KM</option>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  ...
                  <button
                    key={pkg}
                    type="button"
                    onClick={() => onChange("sightseeing_package", pkg)}
                    className={`py-2 px-3 rounded-xl font-bold text-xs border text-center transition-all ${
                      sightseeingPkg === pkg
